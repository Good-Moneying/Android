...
class FifthQuizScreen extends GetView<QuizViewModel> {
  const FifthQuizScreen({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: QuizAppBar(
        onPressed: () {
          controller.init5(true);
          Get.back();
        },
      ),
      body: Obx(() {
        if (controller.init5.value) {
          return _quiz(context);
        } else if (controller.firstQ5.value) {
          return _quizCorrect();
        } else {
          return _quizFalse();
        }
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          GestureDetector(
              onTap: () {
                quizController.init5(false);
                quizController.firstQ5(true);
              },
              child: ChoiceQuiz(number: 'A', detail: '보조금 단가는 올리고, 물량도 늘린다')),
          GestureDetector(
              onTap: () {
                quizController.init5(false);
                quizController.secondQ5(true);
              },
