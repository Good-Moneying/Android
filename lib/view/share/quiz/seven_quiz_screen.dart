...
class SeventhQuizScreen extends GetView<QuizViewModel> {
  const SeventhQuizScreen({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: QuizAppBar(
        onPressed: () {
          controller.init7(true);
          Get.back();
        },
      ),
      body: Obx(() {
        if (controller.init7.value) {
          return _quiz(context);
        } else if (controller.firstQ7.value) {
          return _quizCorrect();
        } else {
          return _quizFalse();
        }
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          GestureDetector(
              onTap: () {
                quizController.init7(false);
                quizController.firstQ7(true);
              },
              child: ChoiceQuiz(number: 'A', detail: '보조금 단가는 올리고, 물량도 늘린다')),
          GestureDetector(
              onTap: () {
                quizController.init7(false);
                quizController.secondQ7(true);
              },
