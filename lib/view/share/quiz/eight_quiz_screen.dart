...
class EightQuizScreen extends GetView<QuizViewModel> {
  const EightQuizScreen({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: QuizAppBar(
        onPressed: () {
          controller.init8(true);
          Get.back();
        },
      ),
      body: Obx(() {
        if (controller.init8.value) {
          return _quiz(context);
        } else if (controller.firstQ8.value) {
          return _quizCorrect();
        } else {
          return _quizFalse();
        }
    ...
    child: Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          GestureDetector(
              onTap: () {
                quizController.init8(false);
                quizController.firstQ8(true);
              },
              child: ChoiceQuiz(number: 'A', detail: '보조금 단가는 올리고, 물량도 늘린다')),
          GestureDetector(
              onTap: () {
                quizController.init8(false);
                quizController.secondQ8(true);
              },
