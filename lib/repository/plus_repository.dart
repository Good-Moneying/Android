...
class PlusRepository {
  ...
  Future<CloudHomeModel> getCloudHome() async {
    ...
    try {
      ...
      final response = await _dio.get("/api/thinkings/home", options: Options(
          headers: {
  ...
  Future<void> postCloudThinking(int thinkingId, List<CloudSentenceModel> sentences) async {
    ...
    try {
      ...
      List<String> sentenceStrings = sentences.map((sentence) => sentence.sentence).toList();
      ...
      if (response.statusCode != 200) {
        throw Exception('Failed to post data!: ${response.statusCode} - ${response.statusMessage}');
      } else {
        print('post 성공!');
      }
    } catch (e) {
